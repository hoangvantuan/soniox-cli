...
- The ref is printed to stderr **before the upload starts**, so it is in the log before anything exists on Soniox. A process killed *after* the upload completed but before the transcription was created leaves an orphaned file carrying that ref: `soniox files list --all` finds it, and the next run reuses it instead of uploading again. Killed *during* the transfer there is nothing to find: the upload is a single request, so Soniox has no file until it finishes.
